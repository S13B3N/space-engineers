...
namespace space_engineers.Autobuild.StorageManagerComplere
{
   public class Program : MyGridProgram, ISmallMainProgram
   {
      ...
      class MISSILE
      {
         ...
         public IMyTerminalBlock GYRO = null;
         public IMyTerminalBlock TURRET = null;
         public IMyTerminalBlock MERGE = null;
         public IMyTerminalBlock POWER = null;
         ...
         {
            bool HasGyro = ( GYRO != null );
            bool HasTurret = ( TURRET != null );
            bool HasMerge = ( MERGE != null );
            bool HasPower = ( POWER != null );
            bool HasThruster = THRUSTERS.Count > 0;
            bool HasWarheads = WARHEADS.Count > 0;

            status = "" + HasGyro + HasTurret + HasMerge + HasPower + HasThruster + HasWarheads;

            return HasGyro && HasThruster && HasWarheads;
            //return HasGyro && HasTurret && HasMerge && HasPower && HasThruster && HasWarheads;
         }

         public string status = "";

         public string EchoSystemDetails ()
         {
            string missileStatus = "Thrusters:"+THRUSTERS.Count+"\n";

            for ( int index = 0; index < THRUSTERS.Count; index++ )
            {
               var thruster = THRUSTERS[index];

               missileStatus += thruster.CustomName+"\n";
            }

            missileStatus += "Warheads:"+WARHEADS.Count+"\n";

            for ( int index = 0; index < WARHEADS.Count; index++ )
            {
               var warhead = WARHEADS[index];

               missileStatus += warhead.CustomName+"\n";
            }

            return missileStatus;
         }

         public string EchoSystemStatusX ()
         {
            bool HasGyro = ( GYRO != null );
            bool HasTurret = ( TURRET != null );
            bool HasMerge = ( MERGE != null );
            ...
            return "Last Missile Failed To Fire\nReason:" +
               "\nHas Gyro: True" +
      ...
      public void Main ( string argument, UpdateType updateSource )
      {
         ...
         else if ( 0 < gatlingShootCount )
         {
            gatlingShootCount--;

            FireGattlingOnce ();
         }
         else if ( bFireMissile )
         {
            Runtime.UpdateFrequency = Global_Timestep == 0.16? UpdateFrequency.Update10 : UpdateFrequency.Update1;
            ...
            GridTerminalSystem.GetBlocksOfType<IMyShipMergeBlock> ( mergeBlockList );
         ...
         else if (( argument == "" ))
         {
            ...
            GridTerminalSystem.GetBlocksOfType<IMyProjector> ( projectorList );

            bool safedistance = false;

            if ( projectorList.Count > 0 )
            {
               safedistance = true;

               IMyProjector projector = projectorList[0];

               foreach ( MISSILE missile in MISSILES )
               {
                  var distance = ( projector.GetPosition () - missile.GYRO.GetPosition ());
      ...
      private void DisplayScriptStatus ()
      {
      ...
      public bool MissileLaunchHandler()
      {
            //Gathers Missile Information
         if ( INIT_NEXT_MISSILE ())
         {
            for ( int index = 0; index < MISSILES.Count; index++ )
            {
               ( MISSILES[index].MERGE as IMyShipMergeBlock ).Enabled = false;
            }
      ...
      bool INIT_NEXT_MISSILE ()
      {
         ...
         GridTerminalSystem.GetBlocksOfType<IMyLargeTurretBase> ( TURRETS, b => b.CustomName.Contains ( "#Turret#" ));
         ...
         GridTerminalSystem.GetBlocksOfType<IMyTerminalBlock> ( BATTERIES, b => b.CustomName.Contains ( MissileTag ) && ( b is IMyBatteryBlock || b is IMyPowerProducer ));
         ...
         foreach ( var Key_Gyro in GYROS )
         {
            ...
            if ( NEW_MISSILE.IsFunctional ())
            {
               MISSILES.Add ( NEW_MISSILE );

               Echo ( NEW_MISSILE.EchoSystemDetails ());
            }
            else
            {
               Lstrundata = NEW_MISSILE.EchoSystemStatusX ();
            }

            Echo ( NEW_MISSILE.status );
         }
      ...
      {
         Echo ( "Missile launched"+MISSILES.Count );

         for ( int missileIndex = 0; missileIndex < MISSILES.Count; missileIndex++ )
         {
            MISSILE ThisMissile = MISSILES[missileIndex];
      ...
      private void ToggleProjector ( bool enabled )
      {
         List<IMyProjector> projectorBlockList = new List<IMyProjector> ();

         GridTerminalSystem.GetBlocksOfType<IMyProjector> ( projectorBlockList );

         for ( int index = 0; index < projectorBlockList.Count; index++ )
         {
            IMyProjector projectorBlock = projectorBlockList[index];

            projectorBlock.Enabled = enabled;
         }
      }


      private void ToggleGasTank ()
      {
         List<IMyGasTank> gasTankList = new List<IMyGasTank> ();
      ...
      private void FireGattlingOnce ()
      {
         List<IMySmallGatlingGun> smallGatlingGunList = new List<IMySmallGatlingGun> ();

         //GridTerminalSystem.GetBlocksOfType<IMySmallGatlingGun> ( smallGatlingGunList, search => search.CustomName.Contains ( "#GAT#" ));
         GridTerminalSystem.GetBlocksOfType<IMySmallGatlingGun> ( smallGatlingGunList );

         for ( int nIndex = 0; nIndex < smallGatlingGunList.Count; nIndex++ )
         {
            IMySmallGatlingGun smallGatlingGun = smallGatlingGunList[nIndex];

            smallGatlingGun.ApplyAction ( "ShootOnce" );
         }
      }
